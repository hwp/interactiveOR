...
usage() {
  echo "Usage: $0 -f frame_rate -v vocabulary_file -r rate -w window_size -s shift_size -n num_banks -c num_coef -o outdir files" >&2
  exit 1
}

cmd="$0 $@"

while getopts "f:v:r:w:s:n:c:o:" opt; do
  case $opt in
    f)
      frate=$OPTARG
      ;;
...
[[ $frate =~ $frre ]] && [[ $rate =~ $nure ]] \
  && [[ $wsize =~ $nure ]] && [[ $ssize =~ $nure ]] \
  && [[ $nbanks =~ $nure ]] && [[ $csize =~ $nure ]] \
...
for file in $files; do
  ...
  gst-launch-1.0 -q filesrc location="$file" ! avidemux name=demux \
    ...
    ! bow vocabulary="$vocfl" location="$video_pipe" ! fakesink \
